...
// Accent color themes (dark mode is handled separately via the Dark Mode toggle)
const ACCENT_THEME_OPTIONS: { id: ThemeType; name: string; swatch: string }[] = [
  { id: 'light', name: 'Default Teal', swatch: Themes.light.primary },
  { id: 'blue', name: 'Ocean Blue', swatch: Themes.blue.primary },
  { id: 'green', name: 'Forest Green', swatch: Themes.green.primary },
  { id: 'purple', name: 'Royal Purple', swatch: Themes.purple.primary },
  { id: 'red', name: 'Ruby Red', swatch: Themes.red.primary },
];

export default function SettingsScreen() {
  const { documents } = useDocuments();
  const {
    accentTheme, colors, isDarkMode, themeMode,
  ...
  const currentAccentName = ACCENT_THEME_OPTIONS.find(t => t.id === accentTheme)?.name || 'Default Teal';
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: colors.background }]} contentContainerStyle={styles.content}>
      ...
      <View style={[styles.card, { backgroundColor: colors.card }]}>
        {/* Dark Mode Section */}
        <SettingRow
          icon="moon-outline" iconColor={colors.primary}
          label="Dark Mode"
          subtitle={themeMode === 'system' ? 'Following system setting' : (isDarkMode ? 'On' : 'Off')}
          right={
            <Switch
              value={isDarkMode}
              onValueChange={(v) => setDarkModeManual(v)}
        ...
        <TouchableOpacity onPress={() => setThemeMode('system')}>
          <SettingRow
            icon="phone-portrait-outline" iconColor={colors.primary}
            label="Match System Theme"
            subtitle="Automatically switch with your device"
            right={themeMode === 'system' ? <Ionicons name="checkmark-circle" size={22} color={colors.primary} /> : <View style={styles.unselectedCircle} />}
          />
        </TouchableOpacity>
        <View style={[styles.divider, { backgroundColor: colors.border }]} />
        {/* Accent Color Picker */}
          ...
          <SettingRow
            ...
            right={
              <View style={styles.swatchRow}>
                <View style={[styles.swatchPreview, { backgroundColor: Themes[accentTheme]?.primary || colors.primary }]} />
      ...
      <View style={[styles.card, { backgroundColor: colors.card }]}>
        <View style={styles.aboutRow}>
          ...
          <View style={styles.aboutInfo}>
            <Text style={[styles.appName, { color: colors.text }]}>Scan Master Pro</Text>
            <Text style={[styles.appVersion, { color: colors.textSecondary }]}>Version 2.1.0</Text>
        ...
        <View style={styles.featureList}>
          {[
            '📷 Edge detection with auto-crop',
            '✂️ Crop, rotate, brightness/contrast',
            '🎨 Color filters & grayscale',
            '🧹 Clean up & sharpen',
            '🖍️ Markup: draw, highlight, arrows, text',
            '🔍 OCR text extraction + copy',
            '📄 Export as PDF/JPG/PNG',
            '🗜️ Compress images & PDFs',
            '🔒 Password-protected PDFs',
            '📎 Real PDF merge with page editor',
            '📁 Organized folders',
      ...
      <Modal visible={showThemeModal} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={[styles.themeModal, { backgroundColor: colors.surface }]}>
            <Text style={[styles.modalTitle, { color: colors.text }]}>Accent Color</Text>
            <Text style={[styles.modalSubtitle, { color: colors.textSecondary }]}>
              Applies to buttons, highlights, and accents across the app.
            </Text>
            {ACCENT_THEME_OPTIONS.map(opt => (
              <TouchableOpacity
                key={opt.id}
                style={[styles.themeOption, { borderColor: colors.border }, accentTheme === opt.id && { borderColor: colors.primary, backgroundColor: colors.primary + '10' }]}
                onPress={() => { setAccentTheme(opt.id); setShowThemeModal(false); }}
              >
                ...
                {accentTheme === opt.id && <Ionicons name="checkmark-circle" size={20} color={colors.primary} />}
              </TouchableOpacity>
            ))}
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.storageModal, { backgroundColor: colors.surface }]}>
            ...
            {[
              ...
              { name: 'compressed/', desc: 'Compressed images and PDFs' },
            ].map(item => (
...
const styles = StyleSheet.create({
  ...
  unselectedCircle: { width: 22, height: 22, borderRadius: 11, borderWidth: 1.5, borderColor: '#CBD5E1' },
  swatchRow: { flexDirection: 'row', alignItems: 'center', gap: Spacing.sm },
  swatchPreview: { width: 22, height: 22, borderRadius: 11 },
  ...
  modalSubtitle: { fontSize: FontSize.sm, marginBottom: Spacing.md, lineHeight: 18 },
