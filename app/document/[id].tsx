...
export default function DocumentDetailScreen() {
  ...
  const handleShare = useCallback(async () => {
    ...
    // Case 1: An export already exists and the file is still present - share it directly
    if (doc.fileUri) {
    ...
    // Case 2: No export yet (e.g. just scanned & saved to gallery, never exported).
    // Auto-generate a shareable file now so the user is never blocked.
    if (doc.pages.length === 0) {
      Alert.alert('Nothing to Share', 'This document has no pages or exported file.');
      return;
    }
    ...
    try {
      ...
      if (doc.format === 'pdf' || doc.pages.length > 1) {
        // Multi-page or PDF target -> build a PDF on the fly
        const uris = doc.pages.map(p => p.uri);
        autoUri = await createPDFFromImages(uris, `${doc.name}_share_${Date.now()}`, doc.dateTimeStamp, doc.password);
      } else {
        // Single page, non-PDF -> share the page image directly
        autoUri = doc.pages[0].uri;
      }

      // Persist this as the document's export so future shares are instant
      ...
      <ScrollView style={[styles.container, { backgroundColor: colors.background }]} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.section, { backgroundColor: colors.surface }]}>
          ...
          <View style={styles.actionsGrid}>
            <TouchableOpacity style={[styles.actionCard, { backgroundColor: colors.surfaceSecondary, borderColor: colors.border }]} onPress={handleShare} disabled={exporting}>
              {exporting ? <ActivityIndicator color={colors.primary} /> : <Ionicons name="share-social-outline" size={28} color={colors.primary} />}
